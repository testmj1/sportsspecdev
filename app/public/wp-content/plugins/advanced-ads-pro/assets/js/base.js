...
var Advads_passive_cb_Conditions = {
	...
	check_referrer_url: function( options ) {
		var referer = advads_pro_utils.extract_cookie_data( advads.get_cookie( this.REFERRER_COOKIE_NAME ) ) || '';
	...
	check_cookie: function( options ) {
		...
		options.value = unescape(options.value.replace(/\\(.)/mg, '$1'));
	...
	check_page_impressions: function( options ) {
		...
		var stored = advads_pro_utils.extract_cookie_data( advads.get_cookie( this.PAGE_IMPRESSIONS_COOKIE_NAME ) );
 ...
 *
 * @return {boolean} true if can be displayed, false otherwise
 */
Advads_passive_cb_Ad.prototype.can_display_by_weekday = function () {
	...
	var date          = new Date(),
		offsetMinutes = window.advanced_ads_pro_ajax_object.wp_timezone_offset / 60,
		offsetHours   = offsetMinutes / 60 >= 0 ? Math.floor(offsetMinutes / 60) : Math.ceil(offsetMinutes / 60);
