( function ( $ ) {
	...
	function addThousandSep( number ) {
		return number.toString().replace( /\B(?=(\d{3})+(?!\d))/g, numberFormats['thousand'] );
	}

	function formatNumber( number, decimals ) {
		var i = parseInt( number );
		var d = parseFloat( number ) - i;

		var result = addThousandSep( i );
		if ( 'undefined' == typeof decimals ) {
			decimals = 0;
		}
		if ( 0 !== d && 0 < decimals ) {
			var D = parseInt( d * ( Math.pow( 10, decimals ) ) ).toString();
	...
	function formatDate( date, format ) {
		...
		var TZ = wpDateTimeZoneName.replace( /([dDjlNSwzWFmMntLoYyaABgGhHisueIOPTZcrU])/g, function ( m, $1, o, s ) {
			return '\\' + $1;
		} );

		// replace any "e" identifier with the escaped WP time zone name - add extra space then trim to avoid complex REGEX processing
		var dateFormat = ' ' + format.replace( /([^\\])(e)/g, function ( m, $1, $2, o, s ) {
			return $1 + TZ;
		} );
	...
	function perDatetable( wrapId, data, dataB, perA, perB ) {
		...
		if ( undefined === dataB ) {
			...
			for ( var date in data ) {
				var ctr     = ( 0 != data[date]['impr'] ) ? ( ( 100 * data[date]['click'] / data[date]['impr'] ) ) : '0.00';
				...
				var row = '<tr><td>' + theDate + '<input type="hidden" value="' + timestamp + '" /></td>' +
					'<td>' + formatNumber( data[date]['impr'] ) + '<input type="hidden" value="' + data[date]['impr'] + '" /></td>' +
					'<td>' + formatNumber( data[date]['click'] ) + '<input type="hidden" value="' + data[date]['click'] + '" /></td>' +
			...
			var totalCTR = ( 0 != totalClick && 0 != totalImpr ) ? ( 100 * totalClick / totalImpr ) + '%' : '0.00%';
			theTable.find( 'tfoot' ).append( $(
				'<tr><th>' + statsLocale.total + '</th>' +
				'<th>' + formatNumber( totalImpr ) + '</th>' +
				'<th>' + formatNumber( totalClick ) + '</th>' +
				'<th>' + formatNumber( totalCTR, 2 ) + '</th></tr>'
			) );
			...
			for ( var date in dataB ) {
				if ( undefined !== rows[index] ) {
					var ctr     = ( 0 != dataB[date]['impr'] ) ? ( ( 100 * dataB[date]['click'] / dataB[date]['impr'] ) ) : '0.00';
					var theDate = new Date( date );
					...
					var extraTD = '<td>' + theDate + '<input type="hidden" value="' + timestamp + '" /></td>' +
						'<td>' + formatNumber( dataB[date]['impr'] ) + '<input type="hidden" value="' + dataB[date]['impr'] + '" /></td>' +
						'<td>' + formatNumber( dataB[date]['click'] ) + '<input type="hidden" value="' + dataB[date]['click'] + '" /></td>' +
						'<td>' + formatNumber( ctr, 2 ) + '%<input type="hidden" value="' + ( ctr * 100 ) + '" /></td>';
			...
			var totalCTRA = ( 0 != totalClickA && 0 != totalImprA ) ? ( 100 * totalClickA / totalImprA ) + '%' : '0.00%';
			var totalCTRB = ( 0 != totalClickB && 0 != totalImprB ) ? ( 100 * totalClickB / totalImprB ) + '%' : '0.00%';
			$( '#' + wrapId ).append( theTable );
			theTable.find( 'tfoot' ).append( $(
				'<tr><th>' + statsLocale.total + '</th>' +
				'<th>' + formatNumber( totalImprA ) + '</th>' +
				'<th>' + formatNumber( totalClickA ) + '</th>' +
				'<th>' + formatNumber( totalCTRA, 2 ) + '</th>' +
				'<th></th>' +
				'<th>' + formatNumber( totalImprB ) + '</th>' +
				'<th>' + formatNumber( totalClickB ) + '</th>' +
				'<th>' + formatNumber( totalCTRB, 2 ) + '</th>' +
				'</tr>'
			) );
			theTable.DataTable( {
				searching: false,
	...
	function perAdtable( wrapId, data, dataB, perA, perB ) {
		...
		if ( undefined === dataB ) {
			...
			for ( var id in data ) {
				...
				var ctr      = ( 0 != data[id]['impr'] ) ? ( ( 100 * data[id]['click'] / data[id]['impr'] ) ) : '0.00';
			...
			theTable.find( 'tfoot' ).append( $(
				...
				'<th>' + formatNumber( totalImprA ) + '</th>' +
				'<th>' + formatNumber( totalClickA ) + '</th>' +
				'<th>' + formatNumber( totalCtrA, 2 ) + '%</th></tr>'
			) );
			$( '#' + wrapId ).append( theTable );
			theTable.DataTable( {
				searching: false,
			...
			for ( var id in dataB ) {
				...
				if ( undefined !== rows[index] ) {
					...
					var ctrB     = ( 0 != dataB[id]['impr'] ) ? ( ( 100 * dataB[id]['click'] / dataB[id]['impr'] ) ) : '0.00';
					var title    = statsLocale.deletedAds;
					if ( 'deleted' != id ) {
						title = adTitles[id];
					};
					var extraTD = '<td>' + formatNumber( dataB[id]['impr'] ) + '<input type="hidden" value="' + dataB[id]['impr'] + '" /></td>' +
						'<td>' + formatNumber( dataB[id]['click'] ) + '<input type="hidden" value="' + dataB[id]['impr'] + '" /></td>' +
						'<td>' + formatNumber( ctrB, 2 ) + '%<input type="hidden" value="' + ctrB + '" /></td>';
					rows[index].append( $( extraTD ) );
			...
			var totalCtrA = ( 0 != totalImprA ) ? ( ( 100 * totalClickA / totalImprA ) ) : '0.00';
			var totalCtrB = ( 0 != totalImprB ) ? ( ( 100 * totalClickB / totalImprB ) ) : '0.00';
			theTable.find( 'tfoot' ).append( $(
				'<tr><th>' + statsLocale.total + '</th>' +
				'<th>' + formatNumber( totalImprA ) + '</th>' +
				'<th>' + formatNumber( totalClickA ) + '</th>' +
				...
				'<th>' + formatNumber( totalImprB ) + '</th>' +
				'<th>' + formatNumber( totalClickB ) + '</th>' +
				'<th>' + formatNumber( totalCtrB, 2 ) + '%</th></tr>'
			) );
			$( '#' + wrapId ).append( theTable );
			theTable.DataTable( {
				searching: false,
	...
	statDisplay.prototype = {
		...
		getGroupFilters: function () {
			return $( '#display-filter-list .display-filter-group' ).map( function () {
		...
		getFilters: function () {
			if ( false !== existingFilter ) {
				...
				return $( '#display-filter-list .display-filter-elem' ).map( function () {
					return parseInt( $( this ).attr( 'data-id' ), 10 );
				} ).get();
			}
		},
				...
				for ( var _date in statsA.impr ) {
					...
					for ( var _ID in statsA.impr[_date] ) {
						...
						if ( this.allAds.indexOf( _ID ) === - 1 ) {
							...
							if ( ! adFilters.length || adFilters.indexOf( parseInt( _ID, 10 ) ) !== - 1 ) {
	...
	$( function () {
		...
		var split1 = numbersFormated.split( '2' )[1].split( '3' );
		if ( '' !== split1[0] ) {
			numberFormats['thousand'] = split1[0];
		}

		var split2 = numbersFormated.split( '5' )[1].split( '6' );
		if ( '' !== split2[0] ) {
