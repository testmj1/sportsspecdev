...
class LeadpagesErrorEvent extends Events
{
    ...
    public function reportError($apiResponse = [], $data = [])
    {
        global $leadpagesApp;

        $adminEmail = get_bloginfo('admin_email');
        $lpUserEmail = get_option('lp-event-email')
            ? get_option('lp-event-email') : '';
    ...
    protected function constructMessage($url, $currentUrl, $apiResponse)
    {
        return "Your blog at {$url} has encountered an error with on of your Leadpages at {$currentUrl} <br />
                ...
                Please contact <a href='https://support.leadpages.net/hc/en-us' target='_blank'>
                Leadpages Support </a> if you continue to have issues.
                ";
    }
}
